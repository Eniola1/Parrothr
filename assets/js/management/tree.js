...
var TREE = (function () {
    ...
    var uID = 0,
        ...
        Tree = function (text, position, parentId, width, height, color, bgcolor, treeData) {
            this.uid = uID += 1;
    ...
    return {
         ...
         */
        create: function (text, position) {
            return new Tree(text, position);
        },
        ...
        destroy: function (tree) {
            ...
            for (var i = 0; i < children.length; i++) {
                if (children[i].uid === tree.uid) {
                    children.splice(i, 1);
                    // var del = children.splice(i, 1);
                    // document.getElementById('position').style.display = "none";
                    // console.log(tree.position, del);
                    break;
                }
            }
        },
        ...
        edit: function (tree) {
            var newText = document.getElementById('sourceText').value;
            if(newText){
            tree.text = newText;
        ...
        draw: function (context, tree) {
            var config = {
                ...
                drawNode = function (tree) {
                    ...
                    context.font = 'bold 10px Source Sans Pro';
                    context.textBaseline = 'middle';
                    context.fillText(text, (x + width / 2) - textWidth / 2, y + height / 2, width);
                    context.strokeStyle = "#1E007F";
