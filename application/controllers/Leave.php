...
class Leave extends CI_Controller
{
    ...
    public function respondedApplications()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $data['employee']    = $this->employee_model->emselect(); // gets active employee details
            $data['leavetypes']  = $this->leave_model->GetleavetypeInfo();
            $data['respondedApplications'] = $this->leave_model->AllLeaveAPPlicationResponded();
            $this->load->view('backend/respondedLeave', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function LeaveApplicationForm()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $data['leavetypes']  = $this->leave_model->GetleavetypeInfo();
            $this->load->view('backend/user/userLeaveApplicationForm', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function Add_Applications()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            // Calculate date difference - leave duration
            $startDate = date_create_from_format('Y-m-d', $data['start_date']);
            $endDate = date_create_from_format('Y-m-d', $data['end_date']);
            // this outputs an array, check docs for all options
            $dateDiff = (array)date_diff($endDate, $startDate);
            $data['leave_duration'] = $dateDiff['days'];
            
            $this->leave_model->Application_Apply($data);
            echo "Successfully Added";

            // Notify Admins 
            // $adminIds = $this->employee_model->getAdminIds();
            // $this->notification->sendGroupNotification($adminIds, 'Employee Leave Request', 'employeeExit/exitPass');
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function updateApplications()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $id           = $this->input->post('id');
            $data         = $this->input->post(array('admin_response'), TRUE);
            $this->leave_model->Application_Apply_Update($id, $data);
                
            echo 'Updated Successfully';

            // Notify Employee
            // $data = $this->leave_model->GetLeaveApply($id);
            // $receiverId = $data->em_id;
            // $this->notification->sendNotification($receiverId, 'Leave Request Update', 'employeeExit/personalExitPassReport');
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    /*Approved and update leave status*/
