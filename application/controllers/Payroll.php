...
class Payroll extends CI_Controller {
   ...
   /* public function Salary_List(){
    ...
    // Show Salary Advance Form
    public function SalaryAdvance()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $this->load->view('backend/user/userSalaryAdvanceForm');
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function AddSalaryAdvance(){     
        if($this->session->userdata('user_login_access') != False) { 
            ...
            $repay_date = $this->input->post('repay_date');
                ...
                $this->payroll_model->addSalaryAdvance($data);
                ...
                // $this->notification->sendGroupNotification($adminIds, 'Salary Advance Request', 'payroll/getSalaryAdvance');
    ...
    /**Get Salary Advance List for admin */
    public function getSalaryAdvance(){
        if($this->session->userdata('user_login_access') != False) { 
            $data['list'] = $this->payroll_model->getSalaryAdvance();
            $this->load->view('backend/salary_advance_list',$data);
        }else{
            redirect(base_url() , 'refresh');
        }
    }
    
    /**Get Salary Advance report for admin */
    public function salaryAdvanceReport(){
        if($this->session->userdata('user_login_access') != False) { 
            $data['list'] = $this->payroll_model->getSalaryAdvanceReport();
            $this->load->view('backend/salary_advance_report',$data);
        }else{
            redirect(base_url() , 'refresh');
        }
    }
    ...
    public function getSalaryAdvanceDataById($id){
        $data = $this->payroll_model->getSalaryAdvanceById($id);
        return $data;
    }
    ...
    public function updateSalaryAdvance(){
        ...
        $this->payroll_model->updateSalaryAdvance($id, $data);
        
        // Send screen alert 
        echo 'Updated Successfully';

        // Notify Employee
        // $data = $this->getSalaryAdvanceDataById($id);
        // $receiverId = $data->emp_id;
        
        // $this->notification->sendNotification($receiverId, 'Salary Advance Update', 'payroll/employeeSalaryAdvance');
        
    }

    /**Get Salary Advance By ID */
    public function getSalaryAdvanceById(){
        $id = $this->input->get('id');
        $data = $this->getSalaryAdvanceDataById($id);
        echo json_encode($data);
    }

    /**Get Employee Salary Advance Request */
    public function employeeSalaryAdvance(){
        if($this->session->userdata('user_login_access') != False) { 
            $id = $this->session->userdata('user_login_id');
            $data['list'] = $this->payroll_model->getSalaryAdvanceByEmployee($id);
            $this->load->view('backend/employee_salary_advance',$data);
        }else{
            redirect(base_url() , 'refresh');
        }
    }
