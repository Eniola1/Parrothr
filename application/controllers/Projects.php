...
class Projects extends CI_Controller
{
	...
	// Get field visit for employees with their ID
	public function EmployeeFieldVisit(){
		if ($this->session->userdata('user_login_access') != False) {
			$id = $this->session->userdata('user_login_id');
			$data['fieldVisits']    = $this->project_model->GetEmployeeFieldVisit($id);
			$this->load->view('backend/employeeFieldVisit', $data);
		} else {
			redirect(base_url(), 'refresh');
		}
	}
	...
	public function Add_Tasks()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			$this->load->library('form_validation');
			$this->form_validation->set_error_delimiters();
			$this->form_validation->set_rules('tasktitle', 'tasktitle', 'trim|required|min_length[3]|max_length[150]|xss_clean');
			$this->form_validation->set_rules('details', 'details', 'trim|required|min_length[4]|max_length[2024]|xss_clean');
				...
				if (empty($id)) {
					...
					if($emid){
						foreach ($emid as $dataarray) {
							...
							$this->project_model->insert_members_Data($data);
							array_push($receiverIds, $dataarray);							
						}
					}
					echo "Successfully Added";

					// Notify Employees invloved
					// $this->notification->sendGroupNotification($receiverIds, 'Task Assignment', 'Projects/employeetasks');
				}
				else {
					...
					// Notify Employees invloved
					// $this->notification->sendGroupNotification($receiverIds, 'Task Assignment Update', 'Projects/employeetasks');
				}
		...
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if (empty($id)) {
					...
					// Notify Employees invloved
					// $this->notification->sendGroupNotification($receiverIds, 'Field Assignment', 'Projects/EmployeeFieldVisit');
				} else {
					...
					// Notify Employees invloved
					// $this->notification->sendGroupNotification($receiverIds, 'Field Assignment Update', 'Projects/EmployeeFieldVisit');
				}
	...
	public function EmployeeTasks()
	{
		if ($this->session->userdata('user_login_access') != False) {
			$id = $this->session->userdata('user_login_id');
			$data['tasks']    = $this->project_model->GetEmployeeTasks($id);
			$this->load->view('backend/employeeTasks', $data);
		} else {
			redirect(base_url(), 'refresh');
		}
	}
	...
	public function FieldVisitDeletByid()
	{
		if ($this->session->userdata('user_login_access') != False) {
            $id= $this->input->post('id');
			$this->project_model->DeleteFieldVisitById($id);
			echo 'Deleted successfully';
		} else {
			redirect(base_url(), 'refresh');
		}
	...
	public function pDelet()
	{
		if ($this->session->userdata('user_login_access') != False) {
			$id    = $this->input->post('id');
			$this->project_model->DletProjectData($id);
			echo 'Deleted successfully';
		} else {
			redirect(base_url(), 'refresh');
		}
	...
	/*Approved or reject the field visit*/
