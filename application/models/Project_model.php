<?php
    ...
    public function GetEmployeeFieldVisit($id){
      $this->db->select('field_visit.*');
      $this->db->select('project.pro_name');
      $this->db->from('field_visit');
      $this->db->join('project', 'project.id = field_visit.project_id','left');
    ...
    public function GetProjectsCompleted(){
        $sql = "SELECT * FROM `project` WHERE `pro_status`='complete'";
    ...
    public function GetTasksValue($id){
      $this->db->select('pro_task.*');
      $this->db->select('assign_task.assign_user');
      $this->db->select('employee.em_id');
      $this->db->select('employee.first_name');
      $this->db->select('employee.last_name');
      $this->db->select('project.pro_name');
      $this->db->select('project.pro_start_date');
      $this->db->select('project.pro_end_date');
      $this->db->from('pro_task');
      $this->db->join('assign_task','pro_task.id = assign_task.task_id','left');
      $this->db->join('employee','assign_task.assign_user = employee.em_id','left');
      $this->db->join('project','pro_task.pro_id = project.id','left');
      $this->db->where('pro_task.id', $id);
      $result = $this->db->get()->row();
		  return $result;          
    }
    
    public function GetEmployeeTasks($id){
      $this->db->select('assign_task.*');
      $this->db->select('project.pro_name');
      $this->db->select('pro_task.*');
      $this->db->from('assign_task');
      $this->db->join("pro_task", "assign_task.task_id = pro_task.id", "left");
      $this->db->join("project", "assign_task.project_id = project.id", "left");
      $this->db->where("assign_user", "$id");
      $query = $this->db->get();
    ...
    //Approved field application - update
    ...
    public function getFieldAuthDataByID($id){
        $sql = "SELECT `field_visit`.*,
      `employee`.`em_id`,`project`.`pro_name`
      FROM `field_visit`
      LEFT JOIN `employee` ON `field_visit`.`emp_id`=`employee`.`em_id` 
      LEFT JOIN `project` ON `field_visit`.`project_id`=`project`.`id` 
    ...
    $sql = "SELECT `assign_task`.*,
      `employee`.`first_name`,`last_name`,`em_image`
      FROM `assign_task`
      LEFT JOIN `employee` ON `assign_task`.`assign_user`=`employee`.`em_id`
    ...
    $sql = "SELECT `assign_task`.*,
      `employee`.`first_name`,`em_image`,`em_id`
      FROM `assign_task`
      LEFT JOIN `employee` ON `assign_task`.`assign_user`=`employee`.`em_id`
      WHERE `assign_task`.`task_id`='$id'";
        $query=$this->db->query($sql);
    ...
    public function DeleteFieldVisitById($id){
        $this->db->delete('field_visit',array('id'=> $id));
    }
