...
*/
    ...
    public function GetLeaveApply($id){
        ...
        $this->db->select('leave_types.name');
        $this->db->from('emp_leave');
        $this->db->where('emp_leave.id', $id);
        $this->db->join('employee', 'employee.em_id = emp_leave.em_id','left');
        $this->db->join('leave_types', 'leave_types.type_id = emp_leave.typeid','left');
    ...
    public function GetallApplication($emid){
        ...
        $this->db->select('employee.first_name');
        $this->db->select('employee.last_name');
        $this->db->select('employee.em_code');
        $this->db->select('leave_types.type_id');
        $this->db->select('leave_types.name');
        $this->db->from('emp_leave');
        $this->db->where('emp_leave.em_id', $emid);
        $this->db->join('employee', 'employee.em_id = emp_leave.em_id','left');
        $this->db->join('leave_types', 'leave_types.type_id = emp_leave.typeid','left');
        $this->db->order_by('emp_leave.id','DESC');
        $result = $this->db->get()->result();
        return $result;
    }
    public function GetBirthdays(){
        //SELECT * FROM employee WHERE WEEK(em_birthday) BETWEEN WEEK( CURDATE() ) AND WEEK( DATE_ADD(CURDATE(), INTERVAL +14 DAY) ) ORDER BY em_birthday ASC LIMIT 5;
        //SELECT * FROM `employee` WHERE month(em_birthday) = month(now()) order by day(em_birthday) desc limit 3;
        //SELECT * from employee where DATEDIFF(em_birthday, now())%365 <= 14 AND day(em_birthday) >= day(now()) ORDER BY day(em_birthday) ASC LIMIT 3;
        $this->db->select('*');
        $this->db->from('employee');
        $this->db->where('DATEDIFF(em_birthday, now())%365 <= 14 AND day(em_birthday) >= day(now())');
        $this->db->order_by('day(em_birthday)','asc');
        $this->db->limit(3);
        $result = $this->db->get()->result();
        return $result;
    }
    public function AllLeaveAPPlication(){
        ...
        $this->db->select('leave_types.name');
        $this->db->from('emp_leave');
        $this->db->where('leave_status', 'Not Approved');
        $this->db->join('employee', 'employee.em_id = emp_leave.em_id','left');
        $this->db->join('leave_types', 'leave_types.type_id = emp_leave.typeid','left');
        $this->db->order_by('emp_leave.id','DESC');
        $result = $this->db->get()->result();
    ...
    public function AllLeaveAPPlicationResponded(){
        $this->db->select('emp_leave.*');
        $this->db->select('employee.first_name');
        $this->db->select('employee.last_name');
        $this->db->select('employee.em_code');
    ...
    public function AllOnLeaveAPPlication(){
        $sql = "SELECT `emp_leave`.*,
      `employee`.`em_id`,`first_name`,`last_name`,`em_code`,
